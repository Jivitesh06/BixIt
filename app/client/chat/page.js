...
import { sendMessage, listenToMessages, listenToBooking, getClientBookings, getBooking } from "@/lib/firestore";
import { SERVICE_CATEGORIES } from "@/lib/constants";
import { getStatusStyle } from "@/lib/utils";
import BottomNav from "@/components/BottomNav";
import { SendIcon, ArrowLeftIcon, ArrowRightIcon, ChatIcon, Spinner } from "@/components/Icons";
...
function ChatListView({ userId }) {
  ...
  useEffect(() => {
    if (!userId) return;
    getClientBookings(userId).then(bookings => {
      const withMsg = bookings.filter(b => b.lastMessage);
      setChats(withMsg); setLoading(false);
    });
  }, [userId]);
      ...
      <div className="px-4 py-4">
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-sm text-[#94A3B8]">Book a worker to start chatting</p>
          </div>
        ) : (
          <div className="space-y-2">
            {chats.map(b => {
              ...
              return (
                <button key={b.id} onClick={() => router.push(`/client/chat?bookingId=${b.id}`)}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-0.5">
                      <p className="font-bold text-[#0F172A] text-sm truncate">{b.workerName || "Worker"}</p>
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col pb-16">
      ...
      <div className="bg-white border-b border-[#E2E8F0] px-4 py-3 sticky top-0 z-40">
        <div className="flex items-center gap-3">
          <button onClick={() => router.push("/client/chat")}
          ...
          <div className="flex-1 min-w-0">
            <p className="font-bold text-[#0F172A] text-sm truncate">{booking?.workerName || "Worker"}</p>
            {booking && (
      ...
      {booking && (
        <div className="mx-4 mt-3 bg-[#FFF7ED] border border-[#FED7AA] rounded-2xl px-4 py-3 flex items-center gap-3">
          ...
          <button onClick={() => router.push(`/client/bookings`)} className="text-[10px] text-[#F97316] font-bold">Details →</button>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
        {messages.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-[#94A3B8] mt-1">Start a conversation with your worker</p>
          </div>
        )}
        {messages.map((msg, i) => {
          ...
          return (
            <div key={msg.id || i} className={`flex flex-col ${isMe ? "items-end" : "items-start"}`}>
              <div className={`max-w-[78%] px-4 py-3 rounded-2xl text-sm leading-relaxed ${isMe ? "bg-[#0F172A] text-white rounded-br-sm" : "bg-white border border-[#E2E8F0] text-[#0F172A] rounded-bl-sm shadow-sm"}`}>
                {msg.text}
              </div>
