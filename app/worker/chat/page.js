      ...
      <div className="flex-1 overflow-y-auto px-4 py-5 space-y-3">
        {messages.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-sm text-[#94A3B8]">Chat with {booking?.clientName || "your client"}</p>
          </div>
        )}
        {messages.map((msg, i) => {
          ...
          return (
            <div key={msg.id || i} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[78%] px-4 py-3 rounded-2xl text-sm leading-relaxed ${isMe ? "bg-[#F97316] text-white rounded-br-md" : "bg-white border border-[#E2E8F0] text-[#0F172A] rounded-bl-md shadow-sm"}`}>
                {msg.text}
                <p className={`text-[10px] mt-1.5 ${isMe ? "text-white/60" : "text-[#94A3B8]"}`}>
