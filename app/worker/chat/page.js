...
import { getStatusStyle, timeAgo, formatCurrency } from "@/lib/utils";
...
export default function WorkerChat() {
  ...
  async function handleSend(e) {
    ...
    const msg = text.trim();
    setText("");
    await sendMessage(bookingId, { senderId: user.uid, senderRole: "worker", text: msg });
  }
    ...
    <div className="min-h-screen bg-[#f7f9fb] flex flex-col">
      ...
      <div className="bg-white border-b border-[#f2f4f6] px-5 py-4 flex items-center gap-3 sticky top-0 z-10">
        ...
        <div className="flex-1 min-w-0">
          <p className="font-headline font-bold text-[#0F172A] truncate">{booking?.clientName || "Client"}</p>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-5 space-y-4 pb-32">
        {messages.length === 0 && !loading && (
          <div className="text-center py-10">
            ...
            <p className="text-sm text-[#45464d]">Start the conversation with your client!</p>
          </div>
        )}
        {messages.map(msg => {
            ...
            <div key={msg.id} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
              <div className="max-w-[78%]">
                <div className={`px-4 py-3 rounded-2xl text-sm leading-relaxed ${
                  isMine ? "bg-[#131b2e] text-white rounded-br-sm" : "bg-[#f2f4f6] text-[#0F172A] rounded-bl-sm"
                }`}>
                  {msg.text}
                </div>
      ...
      <form onSubmit={handleSend}
        ...
        <button type="button" className="w-9 h-9 rounded-full bg-[#f2f4f6] flex items-center justify-center text-[#45464d] flex-shrink-0">
        ...
        <button type="submit" disabled={!text.trim()}
          className="w-10 h-10 rounded-full bg-[#F97316] flex items-center justify-center text-white hover:bg-[#e8680a] disabled:opacity-50 flex-shrink-0 active:scale-95 transition-all">
