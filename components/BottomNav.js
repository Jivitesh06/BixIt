...
const CLIENT_NAV = [
  ...
  { label: "Chat",     href: "/client/chat",       Icon: ChatIcon,  isChatTab: true },
...
const WORKER_NAV = [
  ...
  { label: "Chat",    href: "/worker/chat",       Icon: ChatIcon, isChatTab: true },
...
export default function BottomNav({ role = "client" }) {
  ...
  const { user }  = useAuth();
  const unread    = useUnreadCount(user?.uid, role);
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-[#E2E8F0]"
      ...
      <div className="flex items-center justify-around px-2 pt-2 pb-safe-area">
        {items.map(({ label, href, Icon, isChatTab }) => {
          ...
          return (
            <Link key={href} href={href}
              ...
              <div className="relative">
                ...
                {isChatTab && unread > 0 && (
                  <span className="absolute -top-1 -right-1.5 min-w-[16px] h-4 bg-[#EF4444] text-white text-[9px] font-black rounded-full flex items-center justify-center px-1 leading-none">
                    {unread > 9 ? "9+" : unread}
                  </span>
                )}
              </div>
